...
from phylogeny import build_nj_tree, build_upgma_tree, collect_leaf_names
...
@cli.command()
@click.option("-i", "--input", "input_file", required=True, help="Input aligned FASTA or distance matrix")
@click.option(
    "-m",
    "--method",
...
@click.option("--checkpoint-dir", default=None, help="Directory for checkpoint files")
@click.option("--resume", is_flag=True, default=False, help="Resume from checkpoint")
def tree(input_file, method, output_file, distance_mode, checkpoint_dir, resume):
    """Build phylogenetic tree (NJ/UPGMA) and render ASCII preview."""
    console.print(f"[bold cyan]Loading sequences from {input_file}...[/bold cyan]")
    records = read_sequences(input_file)
    names = [r.id for r in records]
    sequences = [str(r.seq) for r in records]
